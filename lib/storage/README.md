...
> ⚠️ **注意 (v0.16.0 更新)**：以下函数为 mbtgraph **内部实现细节**，
> 外部代码不应依赖它们。它们可能在未来版本中更改或移除而不通知。

### 内部工具函数 (shared_helpers.mbt)
...
| 功能 | 废弃版本 | 替代方案 | 说明 |
|------|:-------:|---------|------|
| `bubble_sort_by_weight()` | v0.14.0 | CSR 内部快速排序 | O(n²) → O(n log n) |

这些功能仍存在于源码中以确保内部兼容性，但不应在新代码中使用。
...
| 版本 | 日期 | 变更 |
|:----:|:----:|------|
| **v0.3.0** | **2026-06-01** | **API 清理：bubble_sort_by_weight 私有化 + int_max 添加使用警告** |
| v0.2.0 | 2026-05-29 | 性能优化：CSR 快排替代冒泡 + 反向索引 + 批量操作 |
| v0.1.0 | 初始版本 | 8 种存储实现 + 泛型转换函数 |
