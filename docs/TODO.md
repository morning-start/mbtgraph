# mbtgraph 任务清单 (TODO)

> **最后更新**: 2026-05-25 | **当前版本**: v0.9.0 | **目标版本**: v0.10.0 🔍 社交网络分析套件
> **下次评审**: 2026-06-01（每周日）

---

## 📋 任务总览

### 当前冲刺：v0.10.0 社交网络分析套件

| 模块 | 状态 | 测试数 | 目标完成 | 进度 |
|------|:----:|:------:|---------|:----:|
| **PageRank** | ⬜ 待开始 | ~15 | 2026-06-01 | **0%** |
| **中心性分析** | ⬜ 待开始 | ~45 | 2026-06-03 | — |
| **社区检测** | ⬜ 待开始 | ~35 | 2026-06-05 | — |
| **文档与集成** | ⬜ 待开始 | — | 2026-06-05 | — |
| **v0.10.0 发布** | ⬜ 待发布 | **~95** | **2026-06-05** | **0%** |

### 版本路线图速查

```
✅ v0.9.0  P5 图论核心完成 (551 tests)
🔨 v0.10.0 🔍 社交网络分析套件     ← 当前目标
⬜ v0.11.0 📊 数据交换与可视化
⬜ v0.12.0 🚀 经典算法增强
⬜ v0.13.0 🛠️ 接口重构
⬜ v1.0.0  🎉 正式发布
```

---

## 🎯 v0.10.0: 🔍 社交网络分析套件

> **时间范围**: 2026-05-25 → 2026-06-05 (2 周)
> **新增测试**: ~95 tests (总计 ~646)
> **核心产出**: PageRank + 中心性分析 + 社区检测

---

### 模块 A: PageRank 算法 [P0]

#### TASK-001: 创建目录结构与包配置

- [ ] 创建 `src/algo/pagerank/` 目录
- [ ] 创建 `moon.pkg` 配置文件（依赖 `@core`）
- [ ] 创建空文件骨架:
  - `types.mbt`
  - `pagerank.mbt`
  - `pagerank_test.mbt`
- **工作量**: 0.5h
- **验收**: `moon check src/algo/pagerank` 通过（空模块编译 OK）

---

#### TASK-002: 实现核心数据结构

- [ ] 定义 `PageRankResult` 结构体:
  ```moonbit
  pub(all) struct PageRankResult {
    ranks : Array[Double]      // 节点排名值
    iterations : Int           // 实际迭代次数
    converged : Bool          // 是否收敛
    damping_factor : Double   // 使用的阻尼系数
    tolerance : Double        // 收敛阈值
  }
  ```
- [ ] 实现 `get_rank(self, node_id) -> Double?` 方法
- [ ] 实现 `top_nodes(self, k) -> Array[(NodeId, Double)]` 方法
- [ ] 实现 `total_rank(self) -> Double` 属性验证方法
- **工作量**: 1h
- **依赖**: TASK-001
- **验收**: 类型定义完整 + 辅助方法可调用

---

#### TASK-003: 实现 PageRank 幂法迭代算法

- [ ] 核心函数签名:
  ```moonbit
  pub fn pagerank[G : @core.GraphReadable](
    graph : G,
    damping_factor : Double,   // 默认 0.85
    max_iterations : Int,       // 默认 100
    tolerance : Double          // 默认 1e-6
  ) -> PageRankResult
  ```
- [ ] 初始化阶段: 均匀分布 rank = 1/N
- [ ] 迭代公式实现:
  ```
  PR(v) = (1-d)/N + d * Σ(PR(u)/out_degree(u))  for u ∈ neighbors(v)
  ```
- [ ] Dangling nodes 处理（无出边节点均匀分配）
- [ ] 收敛判断: ||PR_new - PR_old||∞ < ε
- [ ] 个人化 PageRank 可选支持（personalization vector）
- **复杂度**: O(kE)，k 通常 < 50
- **代码量**: ~120 行
- **工作量**: 3h
- **依赖**: TASK-002
- **验收**: 简单图/星形图/完全图结果正确

---

#### TASK-004: 编写 PageRank 测试套件 (15 tests)

| 分类 | 数量 | 测试项 |
|------|:----:|--------|
| 基础功能 | 5 | 简单图正确性 / 星形图 / 完全图 / 单节点图 / 空图 |
| 参数验证 | 3 | damping factor 范围(0<d<1) / max_iter 截断 / 收敛阈值敏感度 |
| 属性验证 | 4 | rank 总和 ≈ 1 / 对称图均匀分布 / dangling node 处理 / 不可变性 |
| 性能基准 | 3 | 100节点<100ms / 1000节点<1s / 收敛迭代数合理 |
| **合计** | **15** | |

- **工作量**: 2h
- **依赖**: TASK-003
- **验收**: `moon test src/algo/pagerank` 全通过 + 覆盖率 ≥ 90%

---

#### TASK-005: 编写 PageRank 文档

- [ ] `src/algo/pagerank/README.md`:
  - API 使用示例（3 个典型场景）
  - 参数说明表（damping/max_iter/tolerance）
  - 时间/空间复杂度
  - 与 NetworkX 对比说明
- [ ] `docs/design/pagerank_design.md`:
  - 算法数学原理（含公式）
  - 设计决策记录
  - Dangling nodes 处理策略
- **工作量**: 1.5h
- **依赖**: TASK-004
- **验收**: 示例代码可直接运行 + 公式正确

---

### 模块 B: 中心性分析 [P0]

#### TASK-006: 创建目录结构与类型定义

- [ ] 创建 `src/algo/centrality/` 目录
- [ ] 创建 `moon.pkg`（依赖 `@core`）
- [ ] 定义通用结果类型:
  ```moonbit
  pub(all) struct CentralityResult {
    scores : Array[Double]
    normalized : Bool
  }
  ```
- [ ] 创建文件骨架:
  - `degree_centrality.mbt`
  - `betweenness_centrality.mbt`
  - `closeness_centrality.mbt`
  - `eigenvector_centrality.mbt`
  - `centrality_test.mbt`
- **工作量**: 1h
- **验收**: 目录结构完整 + 可编译

---

#### TASK-007: 实现度中心性 (Degree Centrality)

- [ ] 函数签名:
  ```moonbit
  pub fn degree_centrality[G : @core.GraphReadable](
    graph : G,
    mode : DegreeMode  // In/Out/Total
  ) -> CentralityResult
  ```
- [ ] 有向图支持: in_degree / out_degree / total_degree 三种模式
- [ ] 归一化: C(v) = degree(v) / (N-1)
- [ ] 无向图简化: 直接使用 degree
- **复杂度**: O(V+E)
- **代码量**: ~60 行
- **工作量**: 1h
- **依赖**: TASK-006
- **验收**: 星形图中心节点得分最高

---

#### TASK-008: 实现介数中心性 (Betweenness Centrality)

- [ ] 使用 Brandes 算法（O(VE) 比朴素 O(V³) 快）
- [ ] 函数签名:
  ```moonbit
  pub fn betweenness_centrality[G : @core.GraphReadable](
    graph : G,
    normalized : Bool  // 默认 true
  ) -> CentralityResult
  ```
- [ ] 关键步骤:
  1. 对每个节点 s 执行 BFS（或 Dijkstra 加权）
  2. 计算最短路径计数 σ(s,t)
  3. 依赖累积: δ(s,t|v) = (σ_sv/σ_st) * (1 + Σ δ(s,w|w))
  4. 归一化: 2/((N-1)(N-2)) 无向图
- [ ] 支持有向/无向图
- **复杂度**: O(VE)
- **代码量**: ~150 行
- **工作量**: 4h
- **依赖**: TASK-006
- **验收**: 桥节点介数最高 + 与手动计算一致

---

#### TASK-009: 实现接近中心性 (Closeness Centrality)

- [ ] 函数签名:
  ```moonbit
  pub fn closeness_centrality[G : @core.GraphReadable](
    graph : G,
    normalized : Bool
  ) -> CentralityResult
  ```
- [ ] 公式: C(v) = (N-1) / Σ d(v,u)
- [ ] 使用 Floyd-Warshall 或多次 BFS/Dijkstra 计算全源最短路径
- [ ] 不连通图处理: Wasserman Faust 改进（只计算可达节点）
- **复杂度**: O(V(E+VlogV))
- **代码量**: ~80 行
- **工作量**: 2h
- **依赖**: TASK-006
- **验收**: 中心位置节点得分最高

---

#### TASK-010: 实现特征向量中心性 (Eigenvector Centrality)

- [ ] 函数签名:
  ```moonbit
  pub fn eigenvector_centrality[G : @core.GraphReadable](
    graph : G,
    max_iterations : Int,
    tolerance : Double
  ) -> CentralityResult
  ```
- [ ] 幂法迭代（类似 PageRank 但无 damping）:
  - x_v^(k+1) = (1/λ) * Σ x_u^k  for u ∈ neighbors(v)
  - 收敛判断: ||x_new - x_old|| < ε
- [ ] 适用限制: 强连通图效果最佳
- **复杂度**: O(kE)
- **代码量**: ~100 行
- **工作量**: 3h
- **依赖**: TASK-006
- **验收**: 重要枢纽节点得分高 + 收敛稳定

---

#### TASK-011: 编写中心性分析测试套件 (45 tests)

| 算法 | 基础功能 | 边界情况 | 属性验证 | 小计 |
|------|:--------:|:--------:|:--------:|:----:|
| 度中心性 | 4 | 2 | 2 | **8** |
| 介数中心性 | 6 | 3 | 3 | **12** |
| 接近中心性 | 5 | 2 | 3 | **10** |
| 特征向量中心性 | 5 | 2 | 3 | **10** |
| 集成测试 | 5 | — | — | **5** |
| **合计** | **25** | **9** | **11** | **45** |

- **工作量**: 3h
- **依赖**: TASK-007~010
- **验收**: 全部通过 + 覆盖率 ≥ 88%

---

### 模块 C: 社区检测 [P0]

#### TASK-012: 创建目录结构与类型定义

- [ ] 创建 `src/algo/community/` 目录
- [ ] 创建 `moon.pkg`（依赖 `@core`）
- [ ] 定义结果类型:
  ```moonbit
  pub(all) struct CommunityResult {
    labels : Array[Int]         // 节点所属社区标签
    modularity : Double         // 模块度值
    num_communities : Int       // 社区数量
    levels : Int                # Louvain 层数
  }
  ```
- [ ] 创建文件骨架:
  - `louvain.mbt`
  - `label_propagation.mbt`
  - `community_test.mbt`
- **工作量**: 1h
- **验收**: 目录结构完整 + 可编译

---

#### TASK-013: 实现 Louvain 社区检测算法

- [ ] 函数签名:
  ```moonbit
  pub fn louvain[G : @core.GraphReadable](
    graph : G,
    resolution : Double  // 默认 1.0
  ) -> CommunityResult
  ```
- [ ] Phase 1: 贪心 Modularity 最大化
  - ΔQ = [(Σ_in + 2*Σ_tot)/(2m) - (Σ_tot/(2m))^2] - [Σ_in/(2m)]
  - 每次选择 ΔQ 最大的社区移动
  - 仅遍历节点及其邻居（非全图扫描）
- [ ] Phase 2: 超图聚合
  - 社区→新节点
  - 重复 Phase 1 直到 Q 不再提升
- [ ] Resolution 参数可调（影响社区粒度）
- **复杂度**: O(N log N) 近似线性
- **代码量**: ~250 行
- **工作量**: 5h
- **依赖**: TASK-012
- **验收**: Karate Club 图正确分成 2 个社区 + Q ≥ 0.6

---

#### TASK-014: 实现标签传播算法 (Label Propagation)

- [ ] 函数签名:
  ```moonbit
  pub fn label_propagation[G : @core.GraphReadable](
    graph : G,
    max_iterations : Int,  // 默认 100
    seed : Int             // 随机种子（可选）
  ) -> CommunityResult
  ```
- [ ] 初始化: 每个节点唯一标签
- [ ] 异步迭代: 随机顺序更新，采用邻居最多标签
- [ ] 收敛条件: 所有节点标签不再变化 或达到 max_iter
- [ ] 特点: 速度快但结果不稳定（需多次运行取平均）
- **复杂度**: O(kE)，k 通常 < 10
- **代码量**: ~120 行
- **工作量**: 2.5h
- **依赖**: TASK-012
- **验收**: 能识别明显社区结构 + 多次运行一致性 > 80%

---

#### TASK-015: 编写社区检测测试套件 (35 tests)

| 算法 | 经典案例 | 参数验证 | 属性验证 | 性能 | 小计 |
|------|:--------:|:--------:|:--------:|:----:|:----:|
| **Louvain** | 5 | 3 | 4 | 4 | **16** |
| Karate Club / Dolphin / Football | resolution / max_level / 空图 | Q单调递增 / 社区不重叠 / 连通性保持 | 1000节点<5s | |
| **Label Propagation** | 5 | 4 | 3 | 2 | **14** |
| 简单图/不连通图/完全图/空图/单节点 | 种子固定/异步vs同步/max_iter | 社区连通/标签完整性/不可变性 | 5000节点<1s | |
| **集成对比** | 5 | — | — | — | **5** |
| 同一图两种算法结果合理性 | — | — | — | — | |
| **合计** | **15** | **7** | **7** | **6** | **35** |

- **工作量**: 3h
- **依赖**: TASK-013~014
- **验收**: 全部通过 + 覆盖率 ≥ 87%

---

### 集成与发布准备

#### TASK-016: 集成测试与跨模块验证

- [ ] 页面排名 + 中心性组合使用示例
- [ ] 中心性 + 社区检测联动分析
- [ ] 所有存储类型兼容性测试（AdjList/Matrix/CSR/EdgeList）
- [ ] 内存占用检查（无泄漏）
- **工作量**: 2h
- **依赖**: TASK-005, 011, 015
- **验收**: 跨模块场景正常工作

---

#### TASK-017: 项目文档更新

- [ ] AGENTS.md 更新:
  - 添加 pagerank/centrality/community 模块行
  - 更新测试数至 ~646
  - 更新版本号至 v0.10.0
- [ ] MEMORY.md 更新:
  - 添加决策记录（PageRank 幂法/Brandes/Louvain vs GN）
  - 更新包结构
- [ ] README.mbt.md 更新:
  - 算法表添加 3 大模块
  - 统计数据更新
- **工作量**: 1.5h
- **依赖**: TASK-016
- **验收**: 文档一致性检查通过

---

#### TASK-018: Git Tag v0.10.0 发布

- [ ] 最终验证:
  - [ ] `moon check` 零错误零警告 ✅
  - [ ] `moon test` 全通过 (646 tests, 0 failure) ✅
  - [ ] 新模块覆盖率 ≥ 88%
  - [ ] 文档包含使用示例 + 复杂度说明
- [ ] Git 操作:
  ```bash
  git add .
  git commit -m "feat(algo): add social network analysis suite (PageRank + Centrality + Community Detection)"
  git tag -a v0.10.0 -m "feat(v0.10.0): 🔍 Social Network Analysis Suite"
  git push origin main --tags
  ```
- [ ] CHANGELOG.md 新增 v0.10.0 章节
- **工作量**: 1h
- **依赖**: TASK-017
- **验收**: Tag 存在 + GitHub Release 可访问

---

## 📅 开发计划时间线

### 第 1 周 (5.25 - 6.01): PageRank + 中心性基础

| 日期 | 任务 | 交付物 |
|------|------|--------|
| **5.25 (Sun)** | TASK-001~002 | PageRank 目录 + 数据结构 |
| **5.26 (Mon)** | TASK-003 | PageRank 核心算法 |
| **5.27 (Tue)** | TASK-004 + TASK-006~007 | PageRank 测试 + 中心性结构 + 度中心性 |
| **5.28 (Wed)** | TASK-008 | 介数中心性 (Brandes) |
| **5.29 (Thu)** | TASK-009 + TASK-005 | 接近中心性 + PageRank 文档 |
| **5.30 (Fri)** | TASK-010 | 特征向量中心性 |
| **5.31 (Sat)** | TASK-011 | 中心性测试 + 周复盘 |
| **6.01 (Sun)** | Alpha 内部评审 | PageRank + 中心性 完成 ✅ |

### 第 2 周 (6.02 - 6.05): 社区检测 + 集成发布

| 日期 | 任务 | 交付物 |
|------|------|--------|
| **6.02 (Mon)** | TASK-012~013 | 社区检测结构 + Louvain |
| **6.03 (Tue)** | TASK-014 | 标签传播 |
| **6.04 (Wed)** | TASK-015 | 社区检测测试 |
| **6.05 (Thu)** | TASK-016~018 | 集成测试 + 文档 + **v0.10.0 发布** 🎉 |

---

## ✅ 验收标准 (v0.10.0 Final Checklist)

### 功能完整性

- [ ] PageRank: 基本算法 + dangling nodes + 个人化可选
- [ ] 中心性: 度/介数/接近/特征向量 四种指标
- [ ] 社区检测: Louvain + 标签传播 两种算法
- [ ] 所有函数有完整的 `///` 文档注释

### 质量保证

- [ ] `moon test` 全通过 (**~646 tests**, 0 failure)
- [ ] `moon check` 零错误零警告
- [ ] 新模块覆盖率 ≥ **88%**
- [ ] 经典数据集测试通过（Karate Club / Dolphin / Football）

### 文档完整性

- [ ] 每个模块有 README.md（API + 示例 + 复杂度）
- [ ] AGENTS.md / MEMORY.md / README.mbt.md 已更新
- [ ] CHANGELOG.md 有 v0.10.0 条目
- [ ] Git Tag v0.10.0 已创建

### 设计原则遵守

- [ ] 无 `mut self` 参数
- [ ] 无 `for` 元组解构
- [ ] 无嵌套泛型 `>>` 冲突
- [ ] 所有返回值已消费（无 E4139 warning）
- [ ] 数组深拷贝正确（纯函数语义）

---

## 📊 任务统计

| 类别 | 任务数 | 预估工时 | 测试数 |
|------|:------:|:--------:|:------:|
| **PageRank** | 5 (001-005) | 8h | 15 |
| **中心性分析** | 6 (006-011) | 14h | 45 |
| **社区检测** | 4 (012-015) | 11.5h | 35 |
| **集成与发布** | 3 (016-018) | 4.5h | — |
| **合计** | **18 tasks** | **~38h** | **~95 tests** |

---

## 🔄 下一步 (v0.10.0 之后)

完成 v0.10.0 后，按 ROADMAP v2.0.0 继续：

```
v0.11.0 📊 数据交换与可视化 (DOT + JSON + 图统计)   ← 下一个
v0.12.0 🚀 经典算法增强 (A*/费用流/HK/Edmonds/BFS)
v0.13.0 🛠️ 接口重构与 API 冻结准备
...
v1.0.0  🎉 正式发布
```

详细规划见: [ROADMAP.md](./ROADMAP.md)

---

<div align="center">

**💡 提示**: 本文档每周更新，反映最新进展和下周计划

*维护者: @morning-start | 最后更新: 2026-05-25 | 目标: v0.10.0 (2026-06-05)*

</div>
