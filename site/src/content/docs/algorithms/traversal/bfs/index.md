...
Step 1: 出队节点 0，访问其邻居
┌─────────────────────────────────────────────┐
│ Dequeue: 0 → Order: [0]                    │  ← 0 加入访问顺序
│ Enqueue neighbors of 0: [1, 4, 2]          │  ← 未访问的邻居入队
│ Queue: [1, 4, 2]                           │
│ Visited: {0, 1, 4, 2} ✓                    │
│ Level[1]=1, Level[4]=1, Level[2]=1         │  ← 都是第 1 层
│ Parents[1]=0, Parents[4]=0, Parents[2]=0    │
└─────────────────────────────────────────────┘
...
┌─────────────────────────────────────────────┐
│ Dequeue: 1 → Order: [0, 1]                │
│ Neighbors of 1: {0✓, 2✓}                 │  ← 都已访问，跳过
│ Queue: [4, 2]                              │
└─────────────────────────────────────────────┘

Step 3: 出队节点 4
┌─────────────────────────────────────────────┐
│ Dequeue: 4 → Order: [0, 1, 4]              │
│ Neighbors of 4: {0✓, 2✓}                 │  ← 都已访问
│ Queue: [2]                                  │
└─────────────────────────────────────────────┘

Step 4: 出队节点 2 ⭐ 关键步骤！
┌─────────────────────────────────────────────┐
│ Dequeue: 2 → Order: [0, 1, 4, 2]           │
│ Neighbors of 2: {0✓, 1✓, 4✓, 5○}         │
│ → 发现新节点 5!                             │
│ Enqueue: [5]                                │
│ Visited: {..., 5} ✓                         │
│ Level[5] = 2                                │  ← 第 2 层
...
Step 6: 出队节点 3
┌─────────────────────────────────────────────┐
│ Dequeue: 3 → Order: [0, 1, 4, 2, 5, 3]     │
│ Neighbors of 3: {5✓}                       │  ← 已访问
│ Queue: []                                   │  ← 队列为空，BFS 结束!
└─────────────────────────────────────────────┘
```
...
访问顺序 (Order):  [0, 1, 4, 2, 5, 3]
层级信息 (Levels):  L0:[0]  L1:[1,4,2]  L2:[5]  L3:[3]
父节点 (Parents): P[1]=0, P[4]=0, P[2]=0, P[5]=2, P[3]=5
可达节点数: 6 / 6 (全图连通)
