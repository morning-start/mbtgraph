...
|------|--------|:----------:|---------|
| `DirectedAdjList` | `new()` / `new_with_capacity(v,e)` | R+W+D+E | ⭐ 通用有向图 |
| `UndirectedAdjList` | `new()` / `new_with_capacity(v,e)` | R+W+E | ⭐ 通用无向图 |
| `DirectedMatrix` | `new_with_capacity(n)` | R+W+D | 稠密有向小图 |
| `UndirectedMatrix` | `new_with_capacity(n)` | R+W | 稠密无向小图 |
| `EdgeList` | `new()` | R+W+E | Kruskal / 边排序 |
| `UndirectedEdgeList` | `new()` | R+W+E | 无向边排序 |
| `CSR` | `CSRBuilder` → `build()` | R+B | ⭐ 大规模静态有向图 |
| `CSC` | `CSCBuilder` → `build()` | R+B | 大规模入边密集查询 |

> **R**=GraphReadable, **W**=GraphWritable, **D**=GraphDirected, **B**=GraphBatchReadable, **E**=GraphEdgeIterable
...
```moonbit
let g = @storage.UndirectedAdjList::new()
...
```moonbit
let g = @storage.EdgeList::new()
let g = @storage.UndirectedEdgeList::new()
```
