...
const EdmondsKarp = createAlgo<EKStep>({
  legendKeys,
  ...
  generateSteps(
    ...
    steps.push({
      ...
      message: `初始化: 源点=${source}, 汇点=${sink} (Edmonds-Karp: BFS 找最短路增广)`,
    ...
    while (true) {
        ...
        for (const v of adjList[u] || []) {
          ...
          if (!visited.has(v) && residual > 0) {
            ...
            steps.push({
              ...
              message: `BFS 探索: ${u}→${v}, 残量=${residual}/${capacity[k]}`,
              flow: snapshot(flow), capacity: snapshot(capacity),
      ...
      steps.push({
        ...
        message: `✅ 增广路径 (${augPath.length-1} 跳): ${augPath.join('→')}, 推送 ${bottleneck} 单位`,
        flow: snapshot(flow), capacity: snapshot(capacity),
        parent: { ...parent },
    ...
    steps.push({
      type: 'finish', targets: [],
      message: `✅ Edmonds-Karp 完成! 最大流 = ${maxFlow}`,
      flow: snapshot(flow), capacity: snapshot(capacity),
      parent: {}, maxFlow,
    });
  ...
  renderStep(renderer: VizRenderer, step: EKStep, mode: RenderMode, speed: number, colors: ColorMap): void {
    ...
    const all = Object.keys(step.capacity).flatMap(k => k.split('→').map(Number));
    const lastId = String(all.length > 0 ? Math.max(...all) : 0);
  ...
  getUIData(step: EKStep | null, state: UIState): Record<string, string> {
    if (!step) {
      return { 'source': '0', 'sink': '-', 'max-flow': '-', 'edges': '-' };
    }
    return {
      'source': '0',
      'sink': String(Object.keys(step.capacity).flatMap(k => k.split('→').map(Number)).reduce((a, b) => Math.max(a, b), 0)),
      'max-flow': String(step.maxFlow),
      'edges': formatEdges(step.flow, step.capacity),
    };
  },
});

function formatEdges(flow: Record<string, number>, cap: Record<string, number>): string {
  const parts: string[] = [];
  for (const k in cap) {
    if (cap[k] > 0) {
      parts.push(`${k}:${flow[k]||0}/${cap[k]}`);
    }
  }
  return parts.join(', ') || '-';
}
