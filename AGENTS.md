...
description: MoonBit 图算法库 Agent 协作配置（v3.3 新增经典算法增强：A* / 双向 BFS / Hopcroft-Karp / 最小费用最大流 / Edmonds 匹配）
tags: [moonbit, graph-algorithms, trait-based, storage-patterns, network-flow, p5-complete, social-network-analysis, io-module, classic-algorithms-enhanced, doc-system-v3, roadmap-v2]
---
...
| **当前任务清单 (v0.12.0)** | [TODO.md](docs/TODO.md) ✅ |
| **本文档 (Agent 规范)** | 👈 **你在这里** |
...
│   │
│   │  ═════════════════════════════════════════════════
│   │  ✅ 已完成模块 (v0.12.0, 15 子模块, ~659 tests)
│   │  ═════════════════════════════════════════════════
│   │
│   ├── traversal/                # P0: 图遍历 (~58 tests)
│   │   └─ BFS / DFS / 环检测 / 拓扑排序 / 双向 BFS
│   ├── generators/               # P0: 图生成器 (56 tests)
│   │   └─ 16 种随机图生成函数
│   ├── shortest_path/            # P1: 最短路径 (~46 tests)
│   │   └─ Dijkstra / Bellman-Ford / Floyd-Warshall / A* 启发式搜索
│   ├── mst/                      # P2: 最小生成树 (16 tests)
│   │   └─ Kruskal / Prim
│   ├── connectivity/             # P2: 连通性 (21 tests)
│   │   └─ CC / Tarjan SCC / Kosaraju SCC
│   ├── flow/                     # P3: 网络流 (~49 tests)
│   │   └─ Edmonds-Karp / Dinic / 最小费用最大流
│   ├── matching/                 # P4: 图匹配 (~51 tests)
│   │   └─ Hungarian / Hopcroft-Karp / Edmonds 一般图匹配
...
│   ├── pagerank/                 # ✅ 社交网络分析: PageRank (15 tests)
│   │   └─ 幂法迭代 / Dangling nodes / 个人化 PR
│   ├── centrality/               # ✅ 社交网络分析: 中心性 (45 tests)
│   │   └─ 度 / 介数(Brandes) / 接近 / 特征向量
│   ├── community/                # ✅ 社交网络分析: 社区检测 (35 tests)
│   │   └─ Louvain / 标签传播
│   └── integration/              # ✅ 跨模块集成测试 (10 tests)
│       └─ PageRank+中心性+社区检测联动
│
└── io/                            # 🟠 I/O 模块层 (v0.11.0 🆕, 42 tests)
    ├── types.mbt                 # IOError + GraphStats + ConnectivityStats + DegreeDistribution
    ├── dot.mbt                   # DOT 格式读写 (write_dot / parse_dot_into)
    ├── json_serializer.mbt       # JSON 格式读写 (graph_to_json / parse_json_into)
    ├── graph_stats.mbt           # 图统计工具 (basic_stats / degree_distribution / connectivity_stats)
    └── io_test.mbt               # 42 测试 (DOT:20 + JSON:12 + 统计:10)
```
...
| 版本 | 日期 | 主要变更 |
|------|------|---------|
| **v3.3.0** | 2026-05-26 | 🚀 **v0.12.0 发布**: 经典算法增强 (A* / 双向 BFS / Hopcroft-Karp / 最小费用最大流 / Edmonds 匹配) + 71 tests (总计 701) |
| **v3.2.0** | 2026-05-26 | 🚀 **v0.11.0 发布**: 数据交换与可视化 (I/O 模块: DOT + JSON + 图统计) + 42 tests (总计 630) + 项目结构新增 `src/io/` |
| **v3.1.0** | 2026-05-26 | 🚀 **v0.10.0 发布**: 社交网络分析套件 (PageRank + 中心性分析 + 社区检测) + 集成测试 (588 tests, 15 算法模块) |
...
**🤖 mbtgraph Agent 配置 v3.3.0**

*最后更新: 2026-05-26 | 下次评审: 2026-06-09*
